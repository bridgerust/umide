...
impl PluginHostHandler {
    ...
    pub fn handle_notification(
        ...
        match method.as_str() {
            ...
            StartLspServer::METHOD => {
                self.core_rpc.log(
                    umide_rpc::core::LogLevel::Warn,
                    format!(
                        "[{}] Usage of startLspServer as a notification is deprecated.",
                        self.volt_display_name
                    ),
                    Some(format!("umide_proxy::plugin::psp::{}::{}::StartLspServer", self.volt_id.author, self.volt_id.name)),
                );
            ...
            LogMessage::METHOD => {
                ...
                self.catalog_rpc.core_rpc.log_message(
                    message,
                    format!(
                        "umide_proxy::plugin::psp::{}::{}::LogMessage",
