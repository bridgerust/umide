...
mod ffi {
    ...
    pub type FrameCallback =
        extern "C" fn(context: *mut c_void, surface: *mut NativeSurface);

    extern "C" {
        pub fn native_surface_create(
            width: u32,
            height: u32,
            format: u32,
        ) -> *mut NativeSurface;
        pub fn native_surface_resize(
            surface: *mut NativeSurface,
            width: u32,
            height: u32,
        ) -> bool;
        pub fn native_surface_get_iosurface(
            surface: *mut NativeSurface,
        ) -> *mut c_void;
        pub fn native_surface_get_metal_texture(
            surface: *mut NativeSurface,
        ) -> *mut c_void;
        pub fn native_surface_lock(surface: *mut NativeSurface) -> bool;
        pub fn native_surface_unlock(surface: *mut NativeSurface);
        pub fn native_surface_get_buffer(surface: *mut NativeSurface)
            -> *mut c_void;
...
impl MacOSSurface {
    ...
    pub fn new(
        ...
        let ptr =
            unsafe { ffi::native_surface_create(width, height, format as u32) };

        NonNull::new(ptr)
            .map(|ptr| Self { ptr, width, height })
            .ok_or_else(|| {
                NativeSurfaceError::CreationFailed(
                    "IOSurface creation failed".into(),
                )
            })
    }
    ...
    pub fn lock(&self) -> Result<SurfaceLock<'_>, NativeSurfaceError> {
        if unsafe { ffi::native_surface_lock(self.ptr.as_ptr()) } {
            ...
            Err(NativeSurfaceError::OperationFailed(
                "Failed to lock surface".into(),
            ))
        }
    }
...
impl<'a> SurfaceLock<'a> {
    ...
    pub fn buffer_ptr(&self) -> *mut u8 {
        unsafe {
            ffi::native_surface_get_buffer(self.surface.ptr.as_ptr()) as *mut u8
        }
    }
...
impl ScreenCaptureSession {
        ...
        F: Fn(&MacOSSurface) + Send + 'static,
    {
        ...
        let context =
            &*callback_data as *const ScreenCaptureCallbackData as *mut c_void;
        ...
        NonNull::new(ptr)
            ...
            .ok_or_else(|| {
                NativeSurfaceError::CreationFailed(
                    "Failed to start screen capture".into(),
                )
            })
    }
}
...
extern "C" fn screen_capture_trampoline(
    context: *mut c_void,
    surface: *mut ffi::NativeSurface,
) {
