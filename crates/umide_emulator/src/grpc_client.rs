...
use emulator_proto::{
    emulator_controller_client::EmulatorControllerClient, image_format::ImgFormat,
    Image, ImageFormat, Touch, TouchEvent,
};
...
impl EmulatorGrpcClient {
    ...
    pub async fn stream_screenshots(
        ...
        loop {
            match stream.message().await {
                Ok(Some(image)) => match self.image_to_frame(image) {
                    Ok(frame) => {
    ...
    fn image_to_frame(&self, image: Image) -> Result<DecodedFrame, GrpcError> {
        ...
        let (width, height) = if let Some(ref fmt) = image.format {
            let w = if fmt.width > 0 {
                fmt.width
            } else {
                image.width
            };
            let h = if fmt.height > 0 {
                fmt.height
            } else {
                image.height
            };
