...
#[test]
fn test_load_volt() {
    let umide_proxy_dir = std::env::current_dir()
    ...
    let path = umide_proxy_dir.join("some-path");
    ...
    if let Ok(volt_metadata) = load_volt(&umide_proxy_dir) {
        panic!(
            "Unexpected result from `umide_proxy::plugin::wasi::load_volt` function: {volt_metadata:?}"
        );
    }

    // Invalid file (not readable into a string)
    // Making sure the file exists
    let path = umide_proxy_dir.join("smiley.png");
    ...
    let parent_path = umide_proxy_dir.join("some_author.test-plugin-one");
    ...
    let parent_path = umide_proxy_dir.join("some_author.test-plugin-two");
    ...
    let parent_path = umide_proxy_dir.join("some_author.test-plugin-three");
