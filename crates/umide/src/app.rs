...
use floem::{
    ...
    event::{Event, EventListener, EventPropagation, FileDragEvent},
    ...
    prelude::{PointerButtonEvent, SignalTrack},
    reactive::{
        Context, Effect, Memo, ReadSignal, RwSignal, Scope, SignalGet, SignalUpdate,
        SignalWith,
    },
    ...
    unit::PxPctAuto,
    views::{
        Clip, Container, Decorators, Empty, Label, Scroll, Stack, VirtualVector,
        drag_resize_window_area, drag_window_area, dyn_stack,
        editor::{core::register::Clipboard, text::SystemClipboard},
        rich_text,
        scroll::{PropagatePointerWheel, VerticalScrollAsHorizontal},
        svg, tab, tooltip, virtual_stack,
    },
...
use crate::{
    ...
    command::{
        CommandKind, InternalCommand, UmideCommand, UmideWorkbenchCommand,
        WindowCommand,
    },
    config::{
        UmideConfig, color::UmideColor, icon::UmideIcons, ui::TabSeparatorHeight,
...
#[derive(Debug)]
struct Cli {
    /// Launch new window even if Umide is already running
...
impl AppData {
    pub fn reload_config(&self) {
        let config =
            UmideConfig::load(&UmideWorkspace::default(), &[], &self.plugin_paths);
    ...
    fn create_windows(
        ...
        if !dirs.is_empty() {
            ...
            for dir in dirs {
                ...
                {
                    UmideWorkspaceType::RemoteWSL(crate::workspace::WslHost {
                        host: String::new(),
                    })
                } else {
                ...
                #[cfg(not(windows))]
                let workspace_type = UmideWorkspaceType::Local;
                ...
                let db = db.clone();
                app = app.window(
                    move |window_id| app_data.app_view(window_id, info, files, db),
                    Some(config),
                );
                inital_windows += 1;
            }
            ...
            match db.get_app() {
                Ok(app_info) => {
                    for info in app_info.windows {
                        ...
                        app = app.window(
                            move |window_id| {
                                app_data.app_view(window_id, info, vec![], db)
                            },
                            Some(config),
                        );
                        inital_windows += 1;
                ...
                Err(err) => {
                    // Start a new instance if we can't connect to an existing one
                    // ConnectionRefused/NotFound means no existing instance is running
                    let is_expected = err
                    ...
                    if !is_expected {
                        tracing::error!(
                            "Failed to check for existing instance: {:?}",
                            err
                        );
                    } else {
                        tracing::debug!(
                            "No existing instance found, starting new one."
                        );
                    }
                }
            }
        ...
        if inital_windows == 0 {
            let mut info = db.get_window().unwrap_or_else(|_| WindowInfo {
                ...
                tabs: TabsInfo {
                    active_tab: 0,
                    workspaces: vec![UmideWorkspace::default()],
                },
            });
            info.tabs = TabsInfo {
                active_tab: 0,
    ...
    fn app_view(
        ...
        let view = Stack::new((
            ...
            Stack::new((
                drag_resize_window_area(ResizeDirection::West, Empty::new()).style(
                    |s| s.absolute().width(4.0).height_full().pointer_events_auto(),
                ),
                ...
                drag_resize_window_area(ResizeDirection::NorthWest, Empty::new())
                    .style(|s| {
                        s.absolute().width(20.0).height(4.0).pointer_events_auto()
                    }),
                drag_resize_window_area(ResizeDirection::NorthWest, Empty::new())
                    .style(|s| {
                        s.absolute().width(4.0).height(20.0).pointer_events_auto()
                    }),
                drag_resize_window_area(ResizeDirection::NorthEast, Empty::new())
                    .style(move |s| {
                        s.absolute()
                ...
                drag_resize_window_area(ResizeDirection::SouthWest, Empty::new())
                    .style(move |s| {
                        s.absolute()
                            .margin_top(window_size.get().height as f32 - 4.0)
                            .width(20.0)
                ...
                drag_resize_window_area(ResizeDirection::SouthEast, Empty::new())
                    .style(move |s| {
                        s.absolute()
                            .margin_left(window_size.get().width as f32 - 20.0)
                            .margin_top(window_size.get().height as f32 - 4.0)
    ...
    let view_fn = move |(i, layout_rect, child): (
                            ...
                            .apply_if(
                                !info.with(|info| info.is_pristine)
                                    && config.ui.tab_close_button
                                        == TabCloseButton::Off,
                                |s| s.color(config.color(UmideColor::LAPCE_WARN)),
                            )
                    })
            })
            .style(|s| s.padding(4.));
        ...
        .style(move |s| {
            ...
            s.height_full()
                ...
                .hover(|s| s.background(config.color(UmideColor::HOVER_BACKGROUND)))
        })
            ...
            (
                Clip::new(Empty::new().style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .box_shadow_blur(3.0)
                        .box_shadow_color(
                            config.color(UmideColor::LAPCE_DROPDOWN_SHADOW),
                        )
                }))
                .style(move |s| {
                ...
                Stack::new((
                    clickable_icon(
                        || UmideIcons::TAB_PREVIOUS,
                        move || {
                            workbench_command
                                .send(UmideWorkbenchCommand::PreviousEditorTab);
                        },
                        || false,
                        || false,
                        || "Previous Tab",
                    ...
                    clickable_icon(
                        ...
                        move || {
                            workbench_command
                                .send(UmideWorkbenchCommand::NextEditorTab);
                        },
                        || false,
                        || false,
                        || "Next Tab",
        ...
        let child = match child {
            ...
            EditorTabChild::DiffEditor(diff_editor_id) => {
                ...
                if let Some(diff_editor_data) = diff_editor_data {
                    ...
                    let left_editor = RwSignal::new(diff_editor_data.left.clone());
                    let right_editor = RwSignal::new(diff_editor_data.right.clone());
                    Stack::new((
                        Container::new(
                            editor_container_view(
                                window_tab_data.clone(),
            ...
            .style(move |s| {
                ...
                s.position(Position::Absolute)
                    ...
                    .apply_if(is_dragging, |s| {
                        ...
                        .background(config.get().color(UmideColor::EDITOR_CARET))
                    })
                    .hover(|s| {
                        s.cursor(match direction {
                            SplitDirection::Vertical => CursorStyle::ColResize,
...
fn main_split(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.border_color(config.color(UmideColor::LAPCE_BORDER))
...
pub fn clickable_icon_base(
    ...
    let view =
        Container::new(svg(move || config.get().ui_svg(icon())).style(move |s| {
            let config = config.get();
            let size = config.ui.icon_size() as f32;
            s.size(size, size)
                .color(config.color(UmideColor::LAPCE_ICON_ACTIVE))
                .disabled(|s| {
                    s.color(config.color(UmideColor::LAPCE_ICON_INACTIVE))
        ...
        .style(move |s| {
            ...
            s.padding(4.0)
                ...
                .apply_if(active_fn(), |s| {
                    s.border_color(config.color(UmideColor::EDITOR_CARET))
                })
                .hover(|s| {
...
fn tooltip_tip<V: View + 'static>(
    ...
    Container::new(child).style(move |s| {
        ...
        s.padding_horiz(10.0)
            ...
            .background(config.color(UmideColor::TOOLTIP_BACKGROUND))
            .border(1)
            .border_radius(6)
            .border_color(config.color(UmideColor::LAPCE_BORDER))
            .box_shadow_blur(3.0)
            ...
            Container::new(
                Stack::new((
                    ...
                    focus_text(
                        ...
                        move || config.get().color(UmideColor::EDITOR_FOCUS),
                    )
            ...
            Container::new(
                Stack::new((
                    svg(move || {
                        let config = config.get();
                        match mode {
                            RunDebugMode::Run => config.ui_svg(UmideIcons::START),
                            RunDebugMode::Debug => config.ui_svg(UmideIcons::DEBUG),
                        }
                    })
                    .style(move |s| {
                        let config = config.get();
    ...
    .style(move |s| {
        s.width_full()
            ...
            .apply_if(index.get() == i, |style| {
                style.background(
                    config.get().color(UmideColor::PALETTE_CURRENT_BACKGROUND),
                )
            })
    })
}
    ...
    Container::new(
        ...
        .style(move |s| {
            ...
            s.width(config.ui.palette_width() as f64)
                ...
                .border_color(config.color(UmideColor::LAPCE_BORDER))
                .flex_col()
                .background(config.color(UmideColor::PALETTE_BACKGROUND))
                .pointer_events_auto()
        }),
    )
    .style(move |s| {
            ...
            Stack::new((
                svg(move || {
                    if let MessageType::ERROR = message.typ {
                        config.get().ui_svg(UmideIcons::ERROR)
                    } else {
                        config.get().ui_svg(UmideIcons::WARNING)
                    }
                })
                .style(move |s| {
                    let config = config.get();
                    ...
                    let color = if let MessageType::ERROR = message.typ {
                        ...
                        config.color(UmideColor::LAPCE_WARN)
                    };
                    s.min_width(size)
                        .size(size, size)
                        .margin_right(10.0)
...
fn completion(window_tab_data: Rc<WindowTabData>) -> impl View {
                    ...
                    .style(move |s| {
                        ...
                        s.padding_horiz(5.0)
                            ...
                            .apply_if(active.get() == i, |s| {
                                s.background(
                                    config.color(UmideColor::COMPLETION_CURRENT),
                                )
                            })
                            .hover(move |s| {
                                s.background(
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.color(UmideColor::COMPLETION_BACKGROUND))
...
fn code_action(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        .margin_left(origin.x as f32)
        .margin_top(origin.y as f32)
        .background(config.get().color(UmideColor::COMPLETION_BACKGROUND))
        .border_radius(6.0)
    })
...
fn rename(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.get().color(UmideColor::PANEL_BACKGROUND))
            .border_radius(6.0)
            .padding(6.0)
    })
...
fn workspace_title(workspace: &UmideWorkspace) -> Option<String> {
    let p = workspace.path.as_ref()?;
    let dir = p.file_name().unwrap_or(p.as_os_str()).to_string_lossy();
    Some(match &workspace.kind {
        UmideWorkspaceType::Local => format!("{dir}"),
        UmideWorkspaceType::RemoteSSH(remote) => format!("{dir} [{remote}]"),
        #[cfg(windows)]
        UmideWorkspaceType::RemoteWSL(remote) => format!("{dir} [{remote}]"),
    })
}
        ...
        Stack::new((
            ...
            .dragging_style(move |s| {
                ...
                s.border(1.0)
                    ...
                    .color(
                        config
                            .color(UmideColor::EDITOR_FOREGROUND)
                            .multiply_alpha(0.7),
                    )
                    .background(
                        config
            ...
            Empty::new().style(move |s| {
                ...
                s.absolute()
                    ...
                    .height_full()
                    .border_color(
                        config.get().color(UmideColor::LAPCE_TAB_ACTIVE_UNDERLINE),
                    )
...
fn window(window_data: WindowData) -> impl View {
    ...
    .window_title(move || {
        ...
        match workspace {
            Some(workspace) => format!("{workspace} - Umide"),
            None => "Umide".to_string(),
        }
    })
...
pub fn launch() {
    ...
    trace!(TraceLevel::INFO, "Starting up Umide..");
    ...
    let db = match UmideDb::new() {
        Ok(db) => Arc::new(db),
        Err(e) => {
            #[cfg(windows)]
            logging::error_modal("Error", &format!("Failed to create UmideDb: {e}"));

            trace!(TraceLevel::ERROR, "Failed to create UmideDb: {e}");
            std::process::exit(1);
        }
    };
    ...
    let config = UmideConfig::load(&UmideWorkspace::default(), &[], &plugin_paths);
...
pub fn window_menu(
    ...
    menu = menu
        .submenu("Umide", move |m| {
            let mut m = m
                ...
                .item("Quit Umide", |i| {
                    i.action(move || {
                        workbench_command.send(UmideWorkbenchCommand::Quit);
                    })
                });
    ...
    menu = menu.submenu("File", move |m| {
        m.item("New File", |i| {
            i.action(move || {
                workbench_command.send(UmideWorkbenchCommand::NewFile);
            })
        })
        .separator()
        .item("Open", |i| {
            i.action(move || {
                workbench_command.send(UmideWorkbenchCommand::OpenFile);
            })
        })
        .item("Open Folder", |i| {
            i.action(move || {
        ...
        .item("Save All", |i| {
            i.action(move || {
                workbench_command.send(UmideWorkbenchCommand::SaveAll);
            })
        })
        .separator()
        .item("Close Folder", |i| {
            i.action(move || {
                workbench_command.send(UmideWorkbenchCommand::CloseFolder);
            })
        })
        .item("Close Window", |i| {
            i.action(move || {
                workbench_command.send(UmideWorkbenchCommand::CloseWindow);
            })
        })
    });
...
fn tab_secondary_click(
    ...
    let menu = Menu::new()
        ...
        .item("Close Tabs to the Right", |i| {
            i.action({
                let child = child.clone();
                move || {
                    internal_command.send(InternalCommand::EditorTabCloseByKind {
